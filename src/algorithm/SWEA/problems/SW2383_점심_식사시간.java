...
public class SW2383_점심_식사시간 {
    ...
    private static void findDistance(int value) {
        ...
        Point currentStair = stair.get(value);

        for (int i = 0; i < list.size(); i++) {
            if (step[i] == value) {
                Point currentPerson = list.get(i);
                pq.offer(new Point(currentPerson.r, currentPerson.c, calculateDistance(currentPerson, currentStair)));
            }
        }
        ...
        int cnt = 0;
        int time = 0;
        int num = map[currentStair.r][currentStair.c];
